...
$F \equiv 1$ above $d^*(p)$ does \emph{not} hold; instead $\geomCov(p,d) =
q[(1-p)^3 + p^3] - q^2$ with $q = (3 r^2)^d$ in the deep regime $r \le 1/4$, and
...
formula. The companion sphere paper (in preparation) develops this in detail: under
...
$d^*(n,p) = 3 \log n / \log\log n$, sub-logarithmic. The L$^2$ Euclidean torus (Helly
number $d{+}1$) sits between: detection threshold $\Theta(\log n)$ via standard
exponential concentration tail. Together with the present paper (\v{C}ech-Rips collapse
on Helly-2 sup-norm torus, no dimensional barrier), this trilogy gives three distinct
detection-threshold mechanisms unified by the Helly number of the ambient space; a
methodology paper synthesizes them. Extensions to other latent geometries
e.g.\ anisotropic~\cite{Brennan2022} or hyperbolic~\cite{Litvak2022}, where
triangle-based tests behave differently, are also of interest.
