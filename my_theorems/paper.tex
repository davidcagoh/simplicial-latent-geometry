...
\textit{Part~(b).}
The summands $Z_t$ satisfy $|Z_t|\leq 1$, so diagonal terms contribute $O(n^3)$. Pairs
of triangles sharing no vertex or exactly one vertex contribute zero covariance: no shared
vertex gives full independence, and a single shared vertex gives conditional independence
plus translation invariance of the uniform measure on $\TT^d$. The only nontrivial
contribution is from the $O(n^4)$ pairs sharing a common edge, each of which contributes
$O(1)$ covariance. Combining yields $\Var[\tauf] = O(n^4)$. The full case analysis is
deferred to Appendix~\ref{app:variance-bound}.
\end{proof}
...
\paragraph{Simultaneous limits.}
Theorem~\ref{thm:detection} is stated for fixed $d$. A genuine simultaneous-limit statement
---${n,d\to\infty}$ with ${\geomCov(p,d)\to 0}$---would also require a sharper
bound on $\Var_{\Cech}[\tauf]$ than our current $O(n^4)$: the Paley--Zygmund argument needs
${n^2\,\geomCov(p,d)^2\to\infty}$, i.e., ${n\cdot\geomCov(p,d)\to\infty}$, which is strictly
stronger than the SNR criterion ${n^{3/2}\cdot\geomCov(p,d)\to\infty}$ from
Lemma~\ref{lem:snr} once ${\geomCov(p,d) = o(n^{-1/2})}$. Combined with a quantitative
decay rate of $\geomCov$ as above, a tighter variance bound would yield a clean
simultaneous-limit detection theorem.
