...
%% ─────────────────────────────────────────────────────────────────────────────
\section*{Note added in revision (session 63, 2026-05-16)}
%% ─────────────────────────────────────────────────────────────────────────────

\noindent\textbf{Convention-specificity of Theorem~\ref{thm:phase-transition}(b).}
The phase-transition result---a finite $n$-independent critical dimension
$d^*(p) = |\log p|/\log(3/2)$ above which $\geomCov \equiv 0$ via Helly-2 collapse---is
\emph{specific to David's same-radius \v{C}ech convention} (edge and fill both parameterized
by the same radius $r$). N.~Cook (May 2026) flagged that under this convention, the
``\v{C}ech complex'' is \emph{not} a downward-closed simplicial complex (triple-intersection
only forces pairwise distance $\le 2r$, while edges require distance $\le r$). The
natural fix is to reinterpret the fill indicator as the Rips clique
$F_{ijk} = A_{ij} A_{ik} A_{jk}$, which is manifestly downward-closed.

Under the Rips reinterpretation on $\ell_\infty$-torus, Helly-2 forces \v{C}ech and Rips
to coincide \emph{as set complexes}, but the algebraic collapse mechanism behind
Theorem~\ref{thm:phase-transition}(b) does \emph{not} survive: instead,
$\geomCov(p,d) \to p^3(1-p)^3 > 0$ as $d \to \infty$ at fixed $p \in (0,1)$, so the
$\tauf$-based test detects at every dimension and there is no $n$-independent dimensional
barrier. The Theorem~\ref{thm:detection} detection bound and the sparse-regime
Corollary~\ref{cor:sparse} survive intact; only Theorem~\ref{thm:phase-transition}(b)
is convention-dependent.

\noindent\textbf{Sphere sequel (Paper~2).}
The original ``Helly-2 collapse'' headline is recovered, in a stronger form, on the sphere
$S^{d-1}$ with uniform measure under proper \v{C}ech fill (existential common cap; Helly
number $d$). The \v{C}ech-Rips gap survives concentration of measure, and the
$\geomCov_{\text{\v{C}ech}}$ signal vanishes super-exponentially as
$1 - q_{\text{\v{C}ech}}(p,d) \sim (3 z_p^2/d)^{(d-2)/2}$ via the joint Gram-entry density
$\propto (\det G)^{(d-4)/2}$ singularity. The detection threshold is
$d^*(n,p) = 3 \log n / \log\log n$, sub-logarithmic. Companion paper in preparation.

The present paper develops the doubly-signed statistic and detection bound under David's
original convention; readers should treat
Theorem~\ref{thm:phase-transition}(b) and the surrounding ``finite critical dimension''
discussion as a \emph{Helly-2 case study} rather than a structural conclusion. The fuller
multi-instance story---Helly number controls the dimensional barrier; Helly-2 spaces
have no barrier, Helly-$d$ spaces have $\Theta(\log n / \log\log n)$, Helly-$(d{+}1)$
Euclidean torus has $\Theta(\log n)$---is the subject of the methodology paper that
follows this one and its sphere sequel.
...
\item \textbf{Phase transition and finite critical dimension threshold}
...
\caption{The two models on $n=5$ vertices (schematic; the paper uses sup-norm balls on
  ...
  \emph{Right:} Under $\Cech(n,r,d)$, edges connect vertices within distance~$r$ and
  a triple fills whenever the three balls of radius~$r$ share a common point---a
  geometric constraint, not an independent coin flip.}
\label{fig:models}
\end{figure}
...
the indicators decouples from the rest. Above the critical dimension threshold, $F$ becomes
...
\textit{Part~(b).} The critical dimension threshold $d^*(p) = |\log p|/\log(3/2)$ is finite.
...
\begin{definition}[Critical dimension threshold]\label{def:critical-dim}
Define the critical dimension threshold
...
Equivalently, $d^*(p)$ is the smallest positive real number $d$ such that $p^{1/d} \geq 2/3$,
i.e., $r(p,d) \geq 1/3$.
\end{definition}
...
\begin{remark}[Convention-specificity]\label{rem:convention-specific}
The collapse mechanism in part~(b) is specific to David's same-radius \v{C}ech convention.
Under the Rips reinterpretation $F_{ijk} := A_{ij} A_{ik} A_{jk}$, the algebraic identity
$F \equiv 1$ above $d^*(p)$ does \emph{not} hold; instead, for \emph{all}
$p \in (0,1)$ and $d \ge 1$, $\geomCov(p,d) = q[(1-p)^3 + p^3] - q^2$ with
$q = \fillingP(p,d)$ (Lean: \texttt{geometricCov\_eq}, regime-free; in the deep regime
$r \le 1/4$ one has additionally $q = (3 r^2)^d$), and
$\geomCov(p,d) \to p^3 (1-p)^3 > 0$ as $d \to \infty$ at fixed $p$. The
$d^*(p) = |\log p|/\log(3/2)$ headline is therefore a \emph{Helly-2 case study} rather than
a structural barrier. See the ``Note added in revision'' before Section~\ref{sec:intro} for
context, and the sphere sequel for the dimensional-barrier story that survives proper
\v{C}ech complex definition on Helly-$d$ ambient spaces.
\end{remark}
...
\subsection{Quantitative decay rate and the sparse regime}\label{sec:quantitative}

The 1D primitive $\phi(r) = \PP[\text{three arcs of length }2r\text{ on }\TT^1\text{ share a point}]$
admits the closed form
\begin{equation}\label{eq:phi-closed}
  \phi(r) = \begin{cases}
    12r^2 & r \in [0, 1/4], \\
    12r^2 - 3(4r-1)^2 & r \in [1/4, 1/3], \\
    1 & r \geq 1/3,
  \end{cases}
\end{equation}
via Stevens~\cite{Stevens1939}. By coordinate factorisation, $\fillingP(p,d) = \phi(r(p,d))^d$,
and a direct expansion at the threshold $r = 1/3$ gives $\phi(1/3-\epsilon) = 1-36\epsilon^2$
exactly on $\epsilon \in [0,1/12]$. Writing $\eta = 1-d/d^*(p)$ for the fractional gap to the
critical dimension,
\begin{equation}\label{eq:filling-decay}
  1 - \fillingP(p,d) = 4\log(3/2)\,|\!\log p|\,\eta^2\,(1+O(\eta))
  \qquad \text{as } d \nearrow d^*(p).
\end{equation}
Expanding $\geomCov$ into 16 monomials and applying three structural identities (matched marginals,
1D Helly for wedges, all-edges-imply-fill) yields
\begin{equation}\label{eq:geomcov-deep}
  \geomCov(p,d) = (1-q)\,(3r^2)^d + 3p^3\bigl[(7r/2)^d - 1\bigr], \qquad r \leq 1/4.
\end{equation}
The leading term is $\Theta((1-q)\cdot(3/4)^d\cdot p^2)$, decaying exponentially in $d$ but only
polynomially in $p$. The closed forms in~\eqref{eq:phi-closed},~\eqref{eq:geomcov-deep},
and the wedge collapse are formally verified in Lean~4
(\texttt{fillingProb\_eq\_low\_r}, \texttt{geometricCov\_eq\_deep}, \texttt{wedge\_implies\_fill}).%
\footnote{A subsequent strengthening (Lean: \texttt{geometricCov\_eq}, May~2026, Aristotle job
\texttt{bc9f57b3}) proves the regime-free identity
$\geomCov(p,d) = q\bigl[(1-p)^3 + p^3\bigr] - q^2$ with $q = \fillingP(p,d)$ for all
$p \in (0,1)$ and $d \ge 1$, dropping the deep-regime hypothesis $\matchR(p,d) \le 1/4$
required by \texttt{geometricCov\_eq\_deep}. The deep-regime
expansion~\eqref{eq:geomcov-deep} is the specialisation that supplies the explicit
$(3r^2)^d$ and $(7r/2)^d$ factors used in the sparse-regime corollary.}
...
$\Sph^{d-1}$, with an additional dimension threshold $d^*(p_n)$.
...
The lower bounds $\geomCov \geq (1-q)(3/4)^d p^2 - 3p^3$ are currently being
verified in Lean~4 (Aristotle job~\texttt{133c7aac} in progress).
...
\item \emph{Critical dimension threshold.} $\geomCov_k(p,d) = 0$ \emph{exactly} for
...
exactly \textbf{8 named structural axioms}, all documented inline with their mathematical
content:
\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item \texttt{capProb\_neg\_one}, \texttt{capProb\_one},
    \texttt{capProb\_continuous} --- endpoint values and continuity of the
    spherical-cap probability $\Pr_{x \sim \mathrm{Unif}(S^{d-1})}[\langle e_0, x\rangle \ge r]$
    in the threshold $r$ (the cap measure has been concretised via Mathlib's
    \texttt{Measure.toSphere}; these three remaining facts await Cauchy--Schwarz
    norm bounds and the no-atoms property of the inner-product pushforward).
...
\texttt{geomCovCech\_asymptotic} are \emph{derived} from these eight axioms via
a pure analytic squeeze (\texttt{gram\_tail\_bound\_tendsto\_zero}). The axiom
count dropped from 17 (session~70) to 12 (session~77 follow-up,
commits \texttt{b63fc28} concretising \texttt{uniformOnSphere} via
\texttt{Measure.toSphere} + \texttt{51fa5ff} concretising
\texttt{cechFillProb}/\texttt{ripsFillProb}/\texttt{geomCovCech} as iid
product-measure integrals) to the current 8 (commits \texttt{1b1c739} promoting
\texttt{cechFillProb\_le\_one} to a theorem + \texttt{66ae0c5} concretising
\texttt{capProb} and closing \texttt{capProb\_mem\_unitInterval} +
\texttt{capProb\_antitone}).
...
\item \lean{wedge\_implies\_fill}: For any three points $x_1, x_2, x_3$ on $\TT^d$,
  if $A_{12} = 1$ and $A_{13} = 1$ (two edges share vertex $x_1$), then $F_{123} = 1$
  (the triangle is filled). This is the 1D Helly argument: $x_1$ witnesses the triple
  intersection per coordinate. Aristotle job \texttt{43761387}.
...
\smallskip\noindent\textit{OQ-16 Track B — sparse-regime lower bounds (Aristotle job
\texttt{133c7aac}, May~2026).}

\begin{itemize}[leftmargin=*, itemsep=4pt]
\item \lean{geometricCov\_lower\_bound}: $\geomCov(p,d) \geq (1-q)\,\gamma(r)^d - 3p^3$,
  proved by applying \lean{geometricCov\_eq\_deep} and using positivity of $3p^3$ together
  with $(7r/2)^d \leq 1$ for $r \leq 1/4$.

\item \lean{geometricCov\_lower\_bound\_explicit}:
  $\geomCov(p,d) \geq (1-q)\,(3/4)^d\,p^2 - 3p^3$,
  derived from \lean{geometricCov\_lower\_bound} via the matchRadius identity $(2r)^d = p$,
  which gives $(3r^2)^d = (3/4)^d p^2$.
\end{itemize}
