...
The key qualitative difference from BDER is that their detection threshold $d^*\asymp n^3$
grows with~$n$ (edge correlations remain positive for every finite~$d$), while our simplicial
analogue has a \emph{finite, $n$-independent} critical dimension: the fill layer introduces a
complete collapse of geometric signal at a computable threshold, making detection impossible
...
\item \textbf{Detection bound} (Theorem~\ref{thm:detection})\textbf{.}
  If $n^{3/2}\cdot\geomCov(p,d)\to\infty$, the test that thresholds $\tauf$
  achieves asymptotic power $1$: $\TV(\PC,\Cech)\to 1$.

\item \textbf{Phase transition} (Theorem~\ref{thm:phase-transition})\textbf{.}
  There is a finite critical dimension $d^*(p) = |\log p|/\log(3/2)\approx 2.47\,|\log p|$,
  independent of $n$:
  \begin{itemize}[nosep]
    ...
    \item \textbf{Collapsed:} For $d\geq d^*(p)$, $\geomCov(p,d)=0$ exactly
      and no test based on $\tauf$ can distinguish the models.
  \end{itemize}
\end{enumerate}
\end{theorem}

\noindent These two results show that $\tauf$ detects geometry precisely when geometric
covariance is large enough, and that geometric covariance collapses sharply at a computable
threshold determined solely by the edge density~$p$.
...
\paragraph{Power comparison with BDER.}
The fill layer introduces richer geometric dependence than edge structure alone. We conjecture
that the doubly-signed statistic, which exploits two layers of centering, yields detection at
strictly smaller sample sizes than the BDER signed triangle statistic for comparable $(p,d)$,
reflecting the stronger geometric signal encoded in simplicial fills. Making this precise
requires comparing the detection conditions $n^{3/2}\cdot\geomCov(p,d)\to\infty$ and the
analogous BDER condition for the same $(p,d)$ parameters; this reduces to a question about
the relative magnitudes of $\geomCov(p,d)$ and the BDER pairwise covariance.
