...
\smallskip
\noindent\textbf{Instance~1 (Helly-2: $\ell^\infty$ torus, Rips clique fill).}
...
\smallskip
\noindent\textbf{Instance~2 (Helly-$d$: sphere $S^{d-1}$, proper \v{C}ech fill).}
...
barrier. The moment chain, the Paley--Zygmund detection bound (Instance~1 headline
...
the sphere headline reduces to four explicitly named structural axioms.
\end{abstract}
...
This paper bundles the two regimes within a single framework. Instance~1
...
$\geomCov \to p^3(1-p)^3 > 0$. Instance~2 (\S\ref{sec:sphere-instance}, with full Wishart
...
independent. The geometric alternative on the flat torus $\TT^d$ samples $n$ points uniformly,
connects two points within sup-norm distance $r$, and fills a triangle whenever all three
pairwise edges are present (Rips clique fill). The traditional \v{C}ech existential
$F_{ijk} = \mathbf{1}[\bigcap_\ell B(x_\ell, r) \neq \emptyset]$ would seem to be a stronger
geometric condition, but on the $\ell^\infty$ torus the Helly-2 structure (1D Helly per
coordinate) collapses the existential nerve onto the Rips clique fill at the matched-radius
regime (Remark~\ref{rem:helly2-collapse}); we work directly with the Rips clique form
throughout. We retain the symbol $\Cech(n,r,d)$ for continuity with the literature
\cite{Bjorner1995}; see also Figure~\ref{fig:models}. The fundamental detection question is:
given only the observed simplicial complex, can one tell which model generated it?
...
ambients (the $\ell^\infty$ torus, Instance~1), the Rips clique fill is structurally aligned
...
BDER-style SNR threshold. On Helly-$d$ ambients (the sphere, Instance~2), the existential
...
\medskip
\noindent\textbf{Instance~1: $\ell^\infty$ torus, Rips clique fill (Helly-2).}
...
\medskip
\noindent\textbf{Instance~2: sphere $S^{d-1}$, proper \v{C}ech fill (Helly-$d$).}
...
\item \textbf{Lean~4 verification.}
  ...
  The sphere headlines reduce to four explicitly named structural axioms in
...
%% ─────────────────────────────────────────────────────────────────────────────
\section{Instance~1: Detection on the $\ell^\infty$ Torus (Helly-2)}\label{sec:linfty-detection}\label{sec:detection}
%% ─────────────────────────────────────────────────────────────────────────────
...
Instance~1 is restricted to the flat torus $\TT^d$ with the sup-norm metric, where the
...
Instance~2 (sphere instance) requires concentration of measure on $S^{d-1}$ with proper
\v{C}ech fill, and currently reduces to four explicitly named structural axioms in
...
sub-logarithmic dimensional barrier of \S\ref{sec:sphere-instance} to persist,
...
Two natural extensions of the present framework are developed in
\S\ref{sec:sister-instances}: the sphere instance (\S\ref{sec:sphere-instance}), giving
the sub-logarithmic $d^*(n,p) = 3 \log n / \log\log n$ threshold, and the
$\ell^p$-centered-ambient no-go result (\S\ref{sec:lp-nogo}), which shows that the
dimensional-barrier story trivialises on any centered iid-coordinate ambient
(Gaussian, uniform, Bernoulli) under any $\ell^p$ metric. Extensions to other latent
geometries, e.g.\ anisotropic~\cite{Brennan2022} or hyperbolic~\cite{Litvak2022},
where triangle-based tests behave differently, remain open.
...
As of May~2026 (post-session-79 follow-up, branch \texttt{oq-18-rips}) the file declares
exactly \textbf{4 named structural axioms}, all documented inline with their mathematical
content. The earlier analytic obstacles \texttt{capProb\_continuous} (cap probability
continuous in the threshold via radial-cone Fubini + dominated convergence) and
\texttt{matchedCos\_exists} (matched threshold by IVT) are now theorems (Aristotle
\texttt{928ebf69}, commit \texttt{528cec2}, May~2026; \texttt{abeb3eb} for the IVT
half). The remaining axioms are:
\begin{itemize}[leftmargin=*, itemsep=2pt]
  ...
  \item \texttt{cechFillProb\_compl\_le\_gram\_tail} --- the Wishart Gram-density
    ...
    eventually (Instance~2 structural; see Appendix~\ref{app:wishart}).
  \item \texttt{geomCovCech\_decomp\_ratio\_tendsto} --- the algebraic
    ...
    (Instance~2 structural; see Appendix~\ref{app:wishart}).
\end{itemize}
The asymptotic headlines \texttt{cechFillProb\_tail\_asymptotic} and
\texttt{geomCovCech\_asymptotic} are \emph{derived} from these four axioms via
...
\texttt{capProb\_mem\_unitInterval} and \texttt{capProb\_antitone}), to 6
(commits \texttt{391ef9c} and \texttt{29bfcdf} promoting
\texttt{capProb\_neg\_one} and \texttt{capProb\_one} from axioms to theorems),
to 5 (commit \texttt{abeb3eb} promoting \texttt{matchedCos\_exists} via IVT),
and finally to the current 4 (Aristotle \texttt{928ebf69}, commit
\texttt{528cec2}, proving \texttt{capProb\_continuous} via the radial-cone
Fubini argument: codim-1 cones $\{w \mid \langle e_0, w\rangle = c \cdot \|w\|\}$
have Lebesgue measure zero, level sets push forward to $\mathrm{toSphere}$-null
sets, and dominated convergence on the cap indicator gives continuity).
