...
the sphere headline reduces to five explicitly named structural axioms.
\end{abstract}
...
\item \textbf{Lean~4 verification.}
  ...
  The sphere headlines reduce to five explicitly named structural axioms in
...
\v{C}ech fill, and currently reduces to five explicitly named structural axioms in
...
exactly \textbf{5 named structural axioms}, all documented inline with their mathematical
content:
\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item \texttt{capProb\_continuous} --- continuity of the
    ...
    \texttt{29bfcdf}, May~2026), reducing the axiom count from 8 to~6, and the
    matched-threshold existence \texttt{matchedCos\_exists} is now a theorem via IVT
    (commit \texttt{abeb3eb}, May~2026), giving the final 5.
...
\texttt{geomCovCech\_asymptotic} are \emph{derived} from these five axioms via
...
\texttt{capProb\_mem\_unitInterval} and \texttt{capProb\_antitone}), to 6 (commits
\texttt{391ef9c} and \texttt{29bfcdf} promoting \texttt{capProb\_neg\_one} and
\texttt{capProb\_one} from axioms to theorems), and finally to the current 5
(commit \texttt{abeb3eb} promoting \texttt{matchedCos\_exists} via IVT,
May~2026).
